...
func (server *Server) handleConn(w http.ResponseWriter, r *http.Request) {
	...
	if pos := strings.LastIndex(u, "."); pos < 0 || u[pos:] != ".flv" {
		http.Error(w, "invalid path 1", http.StatusBadRequest)
		return
	}
	...
	if len(paths) != 2 {
		http.Error(w, "invalid path 2", http.StatusBadRequest)
		return
	}
	...
	if msgs == nil || len(msgs.Publishers) == 0 {
		http.Error(w, "invalid path 3", http.StatusNotFound)
		return
	} else {
		include := false
		...
		if include == false {
			http.Error(w, "invalid path 5", http.StatusNotFound)
			return
		}
	}
